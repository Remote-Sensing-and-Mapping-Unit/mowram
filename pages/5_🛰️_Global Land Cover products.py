...
geometry = countries.filter(ee.Filter.eq('country_na', 'Cambodia'));
Map.addLayer(geometry, name="Cambodia Boundary")
esa = ee.ImageCollection("ESA/WorldCover/v100").filterBounds(geometry).first().clip(geometry)
esa_vis = {"bands": ["Map"]}


esri = ee.ImageCollection(
    "projects/sat-io/open-datasets/landcover/ESRI_Global-LULC_10m").filterBounds(geometry).mosaic().clip(geometry)
esri_vis = {
    "min": 1,
    "max": 10,
    "palette": [
...
with col2:
    ...
    dw = geemap.dynamic_world(region, start_date, end_date, return_type="hillshade").clip(geometry)
